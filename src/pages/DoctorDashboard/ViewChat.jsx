...
const DoctorViewChat = () => {
  ...
  return (
    // Remove parent padding if any (e.g., from Layout's p-6)
    <div className="h-full flex flex-col bg-gradient-to-br from-blue-50 to-white -m-6">
      ...
      <div className="bg-white/80 backdrop-blur-sm shadow-md flex items-center border-b border-blue-100 flex-shrink-0">
        ...
        <div className="flex items-center gap-3 flex-1 py-3 pr-4">
          {chat.patientProfilePicture ? (
            <img
              ...
              onError={(e) => {
                ...
                e.target.parentNode.innerHTML += `<div class="w-12 h-12 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center text-white text-lg font-bold shadow">${getInitial(chat.patientName)}</div>`;
              }}
            />
          ) : (
          ...
          <div>
            <h2 className="font-semibold text-gray-800">{chat.patientName}</h2>
